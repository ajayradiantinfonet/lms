...
video.id = "video";
//alert($('.video-preview').width());
//video.width = $('.video-preview').width();
//video.height = $('.video-preview').height();
video.width = 250;
video.height=250;
video.autoplay = true;
...
let canvas_pic = document.getElementById("pic_canvas");
...
{
  Promise.all([
    faceapi.nets.tinyFaceDetector.loadFromUri('{% static "account/js/models" %}'),
    faceapi.nets.faceLandmark68Net.loadFromUri('{% static "account/js/models" %}'),
    faceapi.nets.faceRecognitionNet.loadFromUri('{% static "account/js/models" %}'),
    faceapi.nets.faceExpressionNet.loadFromUri('{% static "account/js/models" %}')
  ]).then(startVideo)
}
else 
{
  ...
  //var host=host+":8000";
  let subfolder = 'wink-scroll';

  var scheme = window.location.protocol;
  var url = scheme+"//"+host;

  // Promise.all([
  //   faceapi.nets.tinyFaceDetector.loadFromUri("http://13.232.175.169/lms/LMS/accounts/static/accounts/models"),
  //   faceapi.nets.faceLandmark68Net.loadFromUri("http://13.232.175.169/lms/LMS/accounts/static/accounts/models"),
  //   faceapi.nets.faceRecognitionNet.loadFromUri("http://13.232.175.169/lms/LMS/accounts/static/accounts/models"),
  //   faceapi.nets.faceExpressionNet.loadFromUri("http://13.232.175.169/lms/LMS/accounts/static/accounts/models")
  // ]).then(startVideo);

  Promise.all([
    faceapi.nets.tinyFaceDetector.loadFromUri(url+"/lms/LMS/accounts/static/accounts/models"),
    faceapi.nets.faceLandmark68Net.loadFromUri(url+"/lms/LMS/accounts/static/accounts/models"),
    faceapi.nets.faceRecognitionNet.loadFromUri(url+"/lms/LMS/accounts/static/accounts/models"),
    faceapi.nets.faceExpressionNet.loadFromUri(url+"/lms/LMS/accounts/static/accounts/models")
  ]).then(startVideo);
...
video.addEventListener('play', () => {
  ...
  setInterval(async() => {
    ...
    if(detections.length==1){
      ...
      console.log("only one face in this frame")
    ...
    if(cy > 4 * 150 / 7) 
    {
      //if(DEBUG) {
         console.log("winking .....");
         text.innerHTML = "you blinked just now";
         //cv.imshow('pic_canvas', dst);
         //console.log(cv.imread('canvas2'));

         if (done==false){
             
              ctx_pic.drawImage(video,0,0,canvas_pic.width,canvas_pic.height);
              //imgData = getBase64Image(bannerImage);
              //localStorage.setItem("imgData", imgData);
		var data_url=canvas_pic.toDataURL('image/png');

	document.getElementById("img_save").setAttribute('src',data_url);
              canvas_pic.toBlob(function(blob) {
                localStorage.setItem("image",blob);
		$('.prof').attr('src',URL.createObjectURL(blob));
		$('.prof').show();
		$('#text').hide();
                //var newImg = document.createElement('img'),
                //var a = document.createElement('a');
                //url = URL.createObjectURL(blob);
                //a.href = url, a.download = 'profile_pic';
                ...
                // };
                //newImg.src = url;
                //$('.pic_div').append($('<img>',{id:'theImg',src:url}));
                //console.log(newImg);
              });
              done=true;
         }
         
         // canvas_pic.toBlob() = (blob) => {
         //  const img = new Image();
